...
do
	...
	if [[ $(($RANDOM % 2)) -eq 0 ]]
	then
    	bin/client -f /tmp/socket.sk -l ${CWD}/small_files/small_${SMALL_FILE_NUM}.txt -c ${CWD}/small_files/small_${SMALL_FILE_NUM}.txt -p
	else
    	bin/client -f /tmp/socket.sk -l ${CWD}/medium_files/medium_${MEDIUM_FILE_NUM}.txt -c ${CWD}/medium_files/medium_${MEDIUM_FILE_NUM}.txt -p
	fi
	...
	if [[ $(($RANDOM % 100)) -eq 50 ]]
	then
		bin/client -f /tmp/socket.sk -W ${CWD}/large_files/large_${LARGE_FILE_NUM}.txt -u ${CWD}/large_files/large_${LARGE_FILE_NUM}.txt -p
	fi
